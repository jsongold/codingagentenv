...
- 成功の定義：どの project でも `/pickup` → `/dispatch` が global skill で完遂し、/clear 後も Task list と文脈が残り、未検証の completed が hook で拒否される（handoff の完了条件）
- 絶対に守る業務ルール：
...
## 作業の始め方（/clear後も必ず）
`/pickup` が以下を実行する。
1. .claude/handoff/<name>.md を読む（進行中タスクの状態。handoff が複数あれば選ぶ）。`TaskList` で残タスクを確認する
